...
router.get('/aqi/pasttrends', function(req, res, next) {
    ...
    oracledb.getConnection(
        ...
        function(err, connection)
        {
            ...
            // {
            //     case 1:
            //         finalquery = "SELECT AREA_ID,AQI_TREND,DATE_RECORDED FROM (SELECT P.AREA_ID,CEIL(AVG(P.AQI)) AS AQI_TREND,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM POLLUTION P INNER JOIN AREAS A ON P.AREA_ID=A.AREA_ID WHERE A.AREA_NAME= '" +res.query.data+ "' GROUP BY P.AREA_ID,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM')ORDER BY TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') ASC)WHERE ROWNUM<=10 ORDER BY"
            //
            //
            // }
            // if (err) {
            ...
            connection.execute(
               ...
               // "SELECT AREA_ID,AQI_TREND,DATE_RECORDED FROM (SELECT P.AREA_ID,CEIL(AVG(P.AQI)) AS AQI_TREND,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM POLLUTION P INNER JOIN AREAS A ON P.AREA_ID=A.AREA_ID WHERE A.AREA_NAME=:id GROUP BY P.AREA_ID,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM')ORDER BY TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') DESC)WHERE ROWNUM<=10",
                "SELECT AREA_ID,AQI_TREND,DATE_RECORDED FROM (SELECT P.AREA_ID,ROUND(AVG(P.AQI),2) AS AQI_TREND,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM POLLUTION P INNER JOIN AREAS A ON P.AREA_ID=A.AREA_ID WHERE A.AREA_NAME=:id GROUP BY P.AREA_ID,TO_CHAR(P.DATE_RECORDED, 'YYYY-MM')ORDER BY TO_CHAR(P.DATE_RECORDED, 'YYYY-MM') DESC)WHERE ROWNUM<=10",
                [req.query.data],
                    ...
                    else
                    {
                        ...
                        for(var temp in results.rows){
                            ...
                            if(results.rows[i][1] != null){
                                results.rows[i][1] = Math.round((results.rows[i][1]) * 100)/100 ;
                                console.log(Math.round((results.rows[i][1]) * 100)/100);
                            }
                        ...
                        connection.execute(
                            ...
                            function(err, results2)
                            {
                                ...
                                else
                                {
                                    ...
                                    var count= 0;
                                    for(var temp in results2.rows)
                                        count++;
                                    console.log(count);
                                    var i = 0;
                                    for(var temp in results2.rows){
                                        for(var j =1; j<=6 ; j++){
                                            if(results2.rows[i][j] != null){
                                                results2.rows[i][j] = Math.round((results2.rows[i][j]) * 100)/100 ;
                                                console.log(Math.round((results2.rows[i][j]) * 100)/100);
                                            }
                                        }
                                        i++;
                                    }
                                    console.log("@@@@@@@@@@@@@@@");
                                    console.log(results2.rows);
                                    res.send({chartdata: results2.rows,tabledata: results1.rows});

                                    doRelease(connection);
                                }
                                //console.log(results.metaData);
                                console.log(results2.rows);     // [ [ 180, 'Construction' ] ]
    ...
    oracledb.getConnection(
        ...
        function(err, connection)
        {
            ...
            connection.execute(
                ...
                "SELECT AREA_ID,GQ_TREND,DATE_RECORDED FROM (SELECT T.AREA_ID,ROUND(AVG(T.GQ),2) AS GQ_TREND, TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE A.AREA_NAME=:id GROUP BY T.AREA_ID,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') ORDER BY TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') DESC) WHERE ROWNUM<=10",
                [req.query.data],
...
router.get('/gq/trendtopfive', function(req, res, next) {
    ...
    var actualquery1="SELECT AREA_NAME FROM (SELECT A.AREA_NAME FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID GROUP BY A.AREA_NAME ORDER BY AVG(T.GQ) ASC)WHERE ROWNUM<=5 ";
            ...
            connection.execute(
                ...
                function(err, results1)
                {
                    ...
                    else
                    {
                        ...
                        var flag = true;
                        for(var i in results1.rows){
                            if(results1.rows[i][0] == 'Golden Gate' )
                                flag = false;
                        }
                        ...
                        //
                        // console.log("Done printing new rows");
                        var query1 ="SELECT * FROM (SELECT AREA_NAME ,GQ_TREND ,DATE_RECORDED FROM ( SELECT T.AREA_ID, A.AREA_NAME ,ROUND(AVG(T.GQ),2) AS GQ_TREND,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE A.AREA_NAME IN (( SELECT AREA_NAME FROM (SELECT A.AREA_NAME FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID GROUP BY A.AREA_NAME ORDER BY AVG(T.GQ) ASC)WHERE ROWNUM<=12 OR AREA_NAME='Golden Gate' )) GROUP BY T.AREA_ID,A.AREA_NAME,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') ORDER BY TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') DESC) WHERE ROWNUM<=35) PIVOT (SUM(GQ_TREND) AS GQ_TREND FOR (AREA_NAME)IN (";
                        var query2 =")) ORDER BY DATE_RECORDED desc";
                        var finalquery = query1;
                        for(var i in results1.rows){
                            console.log(results1.rows[i][0]);
                            finalquery = finalquery+"'"+results1.rows[i][0]+"'";
    ...
    oracledb.getConnection(
        ...
        function(err, connection)
        {
            ...
            connection.execute(
                ...
                //"SELECT AREA_NAME,CEIL(GQ),DATE_RECORDED FROM (SELECT A.AREA_NAME,T.GQ,T.DATE_RECORDED FROM GQ_TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE DATE_RECORDED=(SELECT MAX(DATE_RECORDED) FROM GQ_TRENDS) ORDER BY GQ DESC)WHERE ROWNUM<=6 OR AREA_NAME=:id",
                "SELECT AREA_NAME,ROUND(GQ,2),DATE_RECORDED FROM (SELECT A.AREA_NAME,T.GQ,T.DATE_RECORDED FROM GQ_TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE DATE_RECORDED=(SELECT MAX(DATE_RECORDED) FROM GQ_TRENDS) ORDER BY GQ DESC)WHERE ROWNUM<=6 OR AREA_NAME=:id",
                [req.query.data],
        ...
        function(err, connection)
        {
            ...
            connection.execute(
               ...
               // "SELECT AREA_ID ,VQ_TREND ,DATE_RECORDED FROM ( SELECT T.AREA_ID ,CEIL(AVG(T.VQ)) AS VQ_TREND ,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE A.AREA_NAME=:id GROUP BY T.AREA_ID,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') ORDER BY TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') DESC ) WHERE ROWNUM<=10",
                "SELECT AREA_ID ,VQ_TREND ,DATE_RECORDED FROM ( SELECT T.AREA_ID ,ROUND(AVG(T.VQ),2) AS VQ_TREND ,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') AS DATE_RECORDED FROM TRENDS T INNER JOIN AREAS A ON T.AREA_ID=A.AREA_ID WHERE A.AREA_NAME=:id GROUP BY T.AREA_ID,TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') ORDER BY TO_CHAR(T.DATE_RECORDED, 'YYYY-MM') DESC ) WHERE ROWNUM<=10",
