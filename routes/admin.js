...
router.get('/logout', function(req, res, next) {
    req.mySession.reset();
    res.redirect('/admin/login');
    console.log("after session reset"+req.mySession.username);
});

router.get('/updatetree', function(req, res, next) {
    if(!(req.mySession.username==undefined)){
        res.render('admin/updatetree', { title: 'Update Tree' });
    }
    else{
        res.redirect('/admin/login');
    }
                ...
                connection.execute(
                    ...
                    function (err, results) {
                        ...
                        //added else
                            ...
                            for(var temp in results.rows){
                                for(var j =2; j<=3 ; j++){
                                    if(results.rows[i][j] != null){
                                        results.rows[i][j] = Math.round((results.rows[i][j]) * 100)/100 ;
                                        console.log(Math.round((results.rows[i][j]) * 100)/100);
                                    }
                                }
                                i++;
                            }
